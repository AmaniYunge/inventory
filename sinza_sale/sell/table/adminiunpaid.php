 <?php 
 if($_POST['cst'] !=""){
		$querys1=mysql_query("SELECT name,others,age,medicine FROM `siimshealthcenter`.`data` WHERE   id='$_POST[cst]'  ");
	...
	$remain=$remain * -1;
	echo "Change :</td><td>".number_format($remain);
	} 
		echo "</td></tr><table>
		<form id='head2' action='' method='post'>
	pay:<input id='head1' name='pay' type='text' value=''>
	<input name='payid' type='hidden' value='$_POST[unpaid]'>
...
echo "today&nbsp;&nbsp;".  date("Y-m-d")."<br />";
							...
							echo"<table id='tablequery' width='100%'>
							 ...
							 $change_total=0;
							$querys=mysql_query("SELECT sum(cst) as cost,custm,date FROM rdsell   GROUP BY custm  ");
							while($rows=mysql_fetch_array($querys)): 
							$qt=$rows['qt'];
							$name=$rows['custm'];
							$pid=$rows['pid'];

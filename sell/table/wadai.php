...
$dt= "&nbsp;&nbsp;".  date("Y-m-d")."<br />";
 ...
 if($_POST['bnk'] !=""){
  ...
  $query=mysql_query("select * from `moshi_db`.`wadai` where wadai_name='$_POST[id]' ORDER BY id DESC LIMIT 1;");
    ...
    endwhile;
		...
		mysql_query("INSERT INTO `moshi_db`.`wadai` (
`id` ,
`detail` ,
`pokea` ,
`lipa` ,
...
$balance= $balance - $as; 
mysql_query("UPDATE `moshi_db`.`wadai` SET `balance` = '$balance' WHERE `wadai`.`wadai_name` ='$_POST[id]'  ORDER BY id DESC LIMIT 1 ;");
...
mysql_query("UPDATE `moshi_db`.`wadai` SET `balance` = '$balance' WHERE `wadai`.`wadai_name` ='$_POST[id]'  ORDER BY id DESC LIMIT 1 ;");
		...
		$querys1=mysql_query("SELECT *  FROM wadai_name WHERE   id='$_POST[id]'  ");
	...
	datewadai();?>
    <BR /> <BR />
 <button onclick="$('#dep').toggle();">RECIEVED AMOUNT</button> <button onclick="$('#trans').toggle();">RETURN AMOUNT</button>
...
<table id="dep" style="display:none;" width="200" border="0">
  <tr>
    <td colspan="2">RECIEVED AMOUNT</td>
  </tr>
  <tr>
    <td>Detail:</td>
...
<?PHP			
     ...
     <tr bgcolor='#C9CED3'><td >DATE</td><td>DETAIL</td><td>RECIEVED AMOUNT</td><td> RETURNED AMOUNT</td><td>BALANCE</td>
  </tr>
  ";
  $paid="";
  $cost="";
  ...
  $query=mysql_query("select * from wadai where wadai_name='$_POST[id]' AND date BETWEEN '$dateCheck' AND '$dateCheck1' ;");
							while($rows=mysql_fetch_array($query)): 
							$date=$rows['date'];
							$detail=$rows['detail'];
							$deposit=$rows['pokea'];
							$transfer=$rows['lipa'];
							$balance=$rows['balance'];
    echo "<tr style='$td_color border-bottom:thin #ccc solid;' ><td>$date</td><td>$detail</td><td align='right'>".number_format($deposit).
	"</td><td align='right'>".number_format($transfer)."</td><td align='right'>".number_format($balance)."</td></tr>";
...
<input type="hidden" name="dir" value="wadai" />
		...
		mysql_query("INSERT INTO `moshi_db`.`wadai_name` (
`id` ,
`name` ,
`detail`
)
...
//NULL , '', '');
						...
						echo"<table id='tablequery' width='100%'>
							 ...
							 $querys1=mysql_query("SELECT * FROM wadai_name   ");
							while($rowdata=mysql_fetch_array($querys1)):
							$namedata=$rowdata['name'];
							$acc=$rowdata['detail'];
							$id=$rowdata['id'];
						
							$querys=mysql_query("SELECT * from `moshi_db`.`wadai` WHERE   wadai_name='$id' ORDER BY id DESC LIMIT 1  ");

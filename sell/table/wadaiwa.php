...
$dt= "today&nbsp;&nbsp;".  date("Y-m-d")."<br />";
 ...
 if($_POST['bnk'] !=""){
  ...
  $query=mysql_query("select * from `moshi_db`.`wadaiwa` where wadaiwa_name='$_POST[id]' ORDER BY id DESC LIMIT 1;");
    ...
    endwhile;
		...
		mysql_query("INSERT INTO `moshi_db`.`wadaiwa` (
`id` ,
`detail` ,
`pokea` ,
`lipa` ,
...
$balance= $balance - $as; 
mysql_query("UPDATE `moshi_db`.`wadaiwa` SET `balance` = '$balance' WHERE `wadaiwa`.`wadaiwa_name` ='$_POST[id]'  ORDER BY id DESC LIMIT 1 ;");
...
mysql_query("UPDATE `moshi_db`.`wadaiwa` SET `balance` = '$balance' WHERE `wadaiwa`.`wadaiwa_name` ='$_POST[id]'  ORDER BY id DESC LIMIT 1 ;");
		...
		$querys1=mysql_query("SELECT *  FROM wadaiwa_name WHERE   id='$_POST[id]'  ");
   ...
   <?PHP  datewadaiwa();?>
   <br />   <br />
 <button onclick="$('#dep').toggle();">TAKEN AMOUNT</button> <button onclick="$('#trans').toggle();">PAID AMOUNT</button>
...
<?PHP	$no=1;		
     ...
     <tr bgcolor='#C9CED3' style="border:thin solid #ccc;"><td >DATE</td><td>DETAIL</td><td>TEKEN AMOUNT</td><td> RETURNED AMOUNT</td><td>BALANCE</td>
  </tr>
  <?php
  $paid="";
  $cost="";
  ...
  $query=mysql_query("select * from wadaiwa where wadaiwa_name='$_POST[id]' AND date BETWEEN '$dateCheck' AND '$dateCheck1' ;");
							while($rows=mysql_fetch_array($query)): 
							$date=$rows['date'];
							$detail=$rows['detail'];
							$deposit=$rows['pokea'];
							$transfer=$rows['lipa'];
							$balance=$rows['balance'];
    echo "<tr style='$td_color border-bottom:thin #ccc solid;' ><td>$date</td><td>$detail</td>
	<td align='right'>".number_format($deposit).
	"</td><td align='right'>".number_format($transfer)."</td>
...
<input type="hidden" name="dir" value="wadaiwa" />
		...
		mysql_query("INSERT INTO `moshi_db`.`wadaiwa_name` (
`id` ,
`name` ,
`detail`
)
...
//NULL , '', '');
						...
						echo"<div id='div1'> <table id='tablequery' width='100%'>
							 ...
							 $querys1=mysql_query("SELECT * FROM wadaiwa_name   ");
							while($rowdata=mysql_fetch_array($querys1)):
							$namedata=$rowdata['name'];
							$acc=$rowdata['detail'];
							$id=$rowdata['id'];
						
							$querys=mysql_query("SELECT * from `moshi_db`.`wadaiwa` WHERE   wadaiwa_name='$id' ORDER BY id DESC LIMIT 1  ");

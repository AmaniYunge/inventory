...
WHERE name = '$_POST[cst]'   ORDER BY id DESC ");
		...
		$no=1;
							...
							<tr><td colspan='5'>			
									...
									echo "<center><form action='' id='notprint' method='post' >
...
group by name");
							...
							$full=0;
							$cash=0;
						$querys1=mysql_query("SELECT d.name, d.medicine AS cash, d.others AS paid, (
d.medicine - d.others
) AS unpaid
FROM  `siimshealthcenter`.`data` AS d
WHERE d.name =  '$name'
...
");
						  ...
						  $namedata=strtolower($namedata);
											  ...
											  <td align='right'>" .number_format($cash) ."</td>";
											    if(0<$remain){

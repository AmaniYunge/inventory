...
echo "today&nbsp;&nbsp;".  date("Y-m-d")."<br />";
							...
							echo"<table id='tablequery' width='100%'>
								         <th>No.</th>
							            <th align='left' width='300px'>customer name</th>
							 ...
							 $change_total=0;
							$querys=mysql_query("SELECT sum(cst) as cost, sum(profit) as profit ,custm FROM rdsell where  date BETWEEN '$dateCheck' AND '$dateCheck1'  GROUP BY custm  ");
							while($rows=mysql_fetch_array($querys)): 
							$qt=$rows['qt'];
							 $name=$rows['custm'];
							$pid=$rows['pid'];
						...
						$paid_total=$paid_total+$paid;
							...
							echo"<tr style='$td_color border-bottom:thin #ccc solid;' >
							                ...
							                echo " <td align='right'>" .number_format($remaind) ."</td>";
							                  ...
							                  echo " 
											   <td align='right'>" .number_format($ds) ."</td>
							...
							echo"<tr  align='right' height='28px' style='font-size:16px; font:bold; color:#006 ;' >
											...
											echo "</td>
											  <td align='right'>" .number_format($ds_total) ."</td>

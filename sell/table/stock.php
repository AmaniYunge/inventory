...
$querys_=mysql_query("SELECT sum(cst) as cost,sum(qt) as qt,sum(import) as import,sum(profit) as profit FROM rdsell where pid='$id' AND date BETWEEN '2014-01-01' AND CURDATE()  ");
							while($rows_=mysql_fetch_array($querys_)): 
							$qt_=$rows_['qt'];
							$import_=$rows_['import'];
							$quantity=$import_-$qt_;
							$cost=$quantity*$price;
$costby=$quantity*$by;
// if ($no%2)
	//	 {

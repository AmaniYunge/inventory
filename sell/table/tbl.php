...
mysql_query("UPDATE `moshi_db`.`bank` SET `balance` = '$balance' WHERE `bank`.`bankname` ='$_POST[id]'  ORDER BY id DESC LIMIT 1 ;");
	...
	if($_POST['depst']!='' ){
$pallet=$_POST['pallet'];
$e_pallet=$_POST['e_pallet'];
$c_pallet=$e_pallet*$pallet;
$leak=$_POST['leak'];
$e_leak=$_POST['e_leak'];
$c_leak=$leak*$e_leak;
$empties=$_POST['empties'];
$e_empties=$_POST['e_empties'];
$c_empties=$e_empties*$empties;		
...
VALUES (
NULL , '$_POST[depst]', '$_POST[depsamt]', NULL , NULL , CURDATE( ) , '$_POST[id]','$pallet','$e_pallet','$c_pallet','$leak','$e_leak','$c_leak','$empties','$e_empties','$c_empties'
);
");
...
//$balance=// $balance + $_POST['depst'] ;
$balance= $balance + $as+$c_empties+$c_leak+$c_pallet; 
mysql_query("UPDATE `moshi_db`.`bank` SET `balance` = '$balance' WHERE `bank`.`bankname` ='$_POST[id]'  ORDER BY id DESC LIMIT 1 ;");
	...
	?>
    <div class="notprint"> <?php echo datetbl(); ?><br/><br/>
...
<table id="dep" class="notprint" style="display:none;" width="200" border="0">
  ...
  <tr>
    <td>Pallet:</td>
    <td><input type="text" name="pallet" value=""/></td>
  </tr>
  <tr>
    <td>@_pallet:</td>
    <td><input type="text" name="e_pallet" value=""/></td>
  </tr>
  <!--tr>
    <td>Pallet_cost:</td>
    <td><input type="text" name="c_pallet" value=""/></td>
  </tr-->
  <tr>
    <td>Leak:</td>
    <td><input type="text" name="leak" value=""/></td>
  </tr>
  <tr>
    <td>@_leak:</td>
    <td><input type="text" name="e_leak" value=""/></td>
  </tr>
  <tr>
    <!--td>Leak_cost:</td>
    <td><input type="text" name="c_leak" value=""/></td>
  </tr-->
  <tr>
    <td>Empties:</td>
    <td><input type="text" name="empties" value=""/></td>
  </tr>
  <tr>
    <td>@_empties:</td>
    <td><input type="text" name="e_empties" value=""/></td>
  </tr>
  <tr>
    <!--td>Empties_cost:</td>
    <td><input type="text" name="c_emmpties" value=""/></td>
  </tr-->
  <tr>
    <td>&nbsp;</td>
...
<input type="hidden" name="dir" value="tbl"  />
...
require("contact.php");
	...
	echo" Account Name:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$namedata<BR/>
	 Account Detail:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$phonedata<BR/>
	...
	";		
     ...
     <tr bgcolor='#C9CED3' style='border:thin solid #ccc;'>
	 <td >DATE</td><td>DETAILS</td><td>DEPOSIT</td>
	 <td>PALLET</td><td>@_</td><td>COST</td>
	 <td>LEAK</td><td>@_</td><td>COST</td>
	 <td>EMPTIES</td><td>@_</td><td>COST</td><td>TOTAL RETURN COST</td>
	 <td> ORDER</td><td>BALANCE</td>
  </tr>
  ";
  $paid="";
  $cost="";
  ...
  $query=mysql_query("select * from bank where bankname='$_POST[id]' AND date BETWEEN '$dateCheck' AND '$dateCheck1' ;");
							...
							$balance=$rows['balance'];
$pallet=$rows['pallet'];
$e_pallet=$rows['e_pallet'];
$c_pallet=$rows['c_pallet'];
$leak=$rows['leak'];
$e_leak=$rows['e_leak'];
$c_leak=$rows['c_leak'];
$empties=$rows['empties'];
$e_empties=$rows['e_empties'];
$c_empties=$rows['c_empties'];
$t_cost=$c_empties+$c_leak+$c_pallet;
    echo "<tr style='$td_color border:thin #ccc solid;' >
	<td>$date</td>
	<td>$detail</td>
	<td align='right'>".number_format($deposit)."</td>
	<td align='right'>".number_format($pallet)."</td>
	<td align='right'>".number_format($e_pallet)."</td>
	<td align='right'>".number_format($c_pallet)."</td>
		<td align='right'>".number_format($leak)."</td>
 	<td align='right'>".number_format($e_leak)."</td>
	<td align='right'>".number_format($c_leak)."</td>
	<td align='right'>".number_format($empties)."</td>
	<td align='right'>".number_format($e_empties)."</td>
	<td align='right'>".number_format($c_empties)."</td>
	<td align='right'>".number_format($t_cost)."</td>
	<td align='right'>".number_format($transfer)."</td>
	<td align='right'>".number_format($balance)."</td></tr>";
     $t_cost="";
    endwhile;
